...
export default function FranchisePage() {
  ...
  const selectedTeam = selected ? standings.find(t => t.manager === selected) : null;
  const selectedLore = selected ? LEAGUE_LORE.managers[selected as keyof typeof LEAGUE_LORE.managers] : null;
  const selectedS25 = selected ? LEAGUE_LORE.standings2025.find(s => s.manager === selected) : null;
  const selectedChamp = selected ? LEAGUE_LORE.championships.find(c => c.winner === selected) : null;
    ...
    <div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(190px,1fr))',gap:10,marginBottom:24 }}>
        {Object.values(LEAGUE_LORE.managers).map(mgr => {
          ...
          return (
            <div key={mgr.username} onClick={() => setSelected(mgr.username)}
              style={{ background:'linear-gradient(135deg,#001f5b,#1f2d42)', border:`1px solid ${selected===mgr.username?'rgba(245,166,35,0.6)':'var(--bdr)'}`, borderRadius:8, padding:14, cursor:'pointer', transition:'border-color .15s' }}>
              <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:8 }}>
                {team
                  ? <TeamAvatar team={team} size={38} />
                  : <div style={{ width:38,height:38,borderRadius:'50%',background:'var(--s3)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'var(--font-display)',fontSize:13,fontWeight:800,flexShrink:0 }}>{mgr.username.slice(0,2).toUpperCase()}</div>
                }
                <div>
      ...
      {selected && selectedLore && (
        <div>
          <div style={{ background:'linear-gradient(135deg,#001f5b,#1f2d42)',border:'1px solid var(--bdr)',borderRadius:10,padding:20,marginBottom:14 }}>
            <div style={{ display:'flex',alignItems:'center',gap:16,marginBottom:14 }}>
              {selectedTeam
                ...
                : <div style={{ width:60,height:60,borderRadius:'50%',background:'var(--s3)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'var(--font-display)',fontSize:20,fontWeight:900 }}>{selected.slice(0,2).toUpperCase()}</div>
              }
              <div>
                ...
                {selectedChamp && <div style={{ fontSize:11,color:'var(--gold)' }}>🏆 {selectedChamp.season} Champion</div>}
              </div>
            </div>
            ...
            {selectedS25 && (
              <div style={{ marginTop:12,padding:'10px 12px',background:'rgba(0,0,0,0.2)',borderRadius:6 }}>
                ...
                <div style={{ fontSize:13,fontWeight:600 }}>Final Rank: #{selectedS25.rank}</div>
                <div style={{ fontSize:11,color:'var(--muted)',marginTop:2 }}>{selectedS25.notes}</div>
              </div>
            )}
            ...
            {selectedTeam && (
              <div style={{ marginTop:10,display:'flex',gap:18,flexWrap:'wrap' }}>
                {([
                  ['2026 Record', `${selectedTeam.wins}-${selectedTeam.losses}`],
                  ['PF', selectedTeam.pf.toFixed(1)],
                  ['PPG', selectedTeam.avgPpg.toFixed(1)],
                  ['Rank', `#${selectedTeam.rank}`],
                  ['Luck', `${selectedTeam.luckScore > 0 ? '+' : ''}${selectedTeam.luckScore}`],
          ...
          <AIContentLoader
            key={selected}
            type="franchise"
            params={{ manager: selected }}
            label="FIN FRANCHISE PROFILE"
            personaName="Michelle Carter"
          />
        </div>
      )}
    </div>
